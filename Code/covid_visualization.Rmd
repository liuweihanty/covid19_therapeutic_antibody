...
```{r}
sub_piece <- country[c(1,ncol(country))]
for (i in 2:(ncol(country)-1)) {
    sub_piece_next <- country[c(i,ncol(country))]
...
sub_piece$Status <- factor(sub_piece$Status, levels = c("Phase 3","Phase 2/3","Phase 2","Phase 1/2","Phase 1","Preclinical"))
...
country_plot <- ggplot(data = sub_piece, aes(x = fct_infreq(country), y = count, fill = Status)) + 
    ...
    ylab("numbers of antibody therapeutics in development") +
    xlab("Countries") +
